...
const ownerId = 'your-discord-user-id'; // Replace with your Discord user ID
...
function sendMaintenanceMessage(message) {
    message.reply('The bot is currently under maintenance and cannot respond to commands at the moment. Please try again later.');
}
...
client.on('messageCreate', (message) => {
    ...
    if (message.content.toLowerCase().startsWith('!maintenance') && message.author.id === ownerId) {
        ...
        if (action === 'on') {
            ...
            message.reply('Please use `!maintenance on` to enable or `!maintenance off` to disable maintenance mode.');
        }
    ...
    if (message.content.toLowerCase().startsWith('!status')) {
        ...
        if (!status) {
            return message.reply('Please provide a status: `online`, `dnd`, `idle`, or `invisible`.');
        }
        ...
        switch (status.toLowerCase()) {
            ...
            case 'invisible':
                ...
                break;
            default:
                message.reply('Invalid status. Please use `online`, `dnd`, `idle`, or `invisible`.');
                break;
        }
        return;
    }
